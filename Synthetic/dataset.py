...
class MultiConcept_w_Features(MultimodalDataset):
    def __init__(self, total_data, total_labels, total_concepts, concept_id, total_raw_features):
        self.data = torch.from_numpy(total_data).float()
        self.labels = torch.from_numpy(total_labels).float()
        self.concepts = torch.from_numpy(total_concepts).float()
        self.concept_id = concept_id
        self.features = total_raw_features

    def __getitem__(self, idx):
        ite = tuple([self.data[idx,:]]+[self.concepts[self.concept_id,idx,:]]+[self.labels[idx]]+
                    [dict(zip(self.features.keys(), [torch.from_numpy(self.features[key][idx]).float() for key in self.features.keys()]))])
        return ite
...
def generate_data_concepts(num_data, num_concepts, feature_dim_info, label_dim_info, transform_dim = 100,
    ...
    if transforms_2hd is None:
        transforms_2hd = mlp(concepts_dims[0], 512, transform_dim, layers=2, activation='tanh') # int(transform_dim/2.0 =512 #concepts_dims[0] #total_dims
        #transforms_2hd = (np.random.uniform(0.0,1.0,(total_dims, transform_dim)))

    if transforms_2concept is None:
        transforms_2concept = []
        for i in range(num_concepts):
            transforms_2concept.append(torch.rand(concepts_dims[i], 1).to('cuda'))
            # transforms_2concept.append(np.random.uniform(0.0,1.0,(concepts_dims[i], 1)))
            # transforms_2concept.append(mlp(concepts_dims[i]+44, 4, 1, layers=2, activation='tanh').to('cuda')) #concepts_dims[i]+512
    ...
    for data_idx in range(num_data):
        ...
        irrelevant_info_generator = torch.distributions.multivariate_normal.MultivariateNormal(torch.zeros((22,)), torch.eye(22)*0.5)
        ...
        concepts_labels = [torch.tensor(raw_data[i], dtype=torch.float).to('cuda') @ transforms_2concept[i] for i in range(num_concepts)]
        # concepts_labels = [torch.cat((torch.tensor(raw_data[i], dtype=torch.float).to('cuda'), w3, w4))@ transforms_2concept[i] for i in range(num_concepts)]
        concepts_labels = [concepts_labels[i].detach().cpu().numpy() for i in range(num_concepts)]
        ...
        ##change raw total to exclude c
        input_means = []
        for i in range(2, num_concepts+2):
            input_means.append([])
            for k, v in raw_features.items():
                if str(i) in k:
                    input_means[-1].append(v)
                    
        raw_input = [np.concatenate(input_means[i]) for i in range(len(input_means))] 
        raw_total = raw_input[0] #np.concatenate([v for v in raw_features.values()]) # 
        ...
        # total_x = transforms_2hd(torch.cat((raw_total,w3))).detach().cpu().numpy()
        # total_x = torch.cat((total_x, w3)).detach().cpu().numpy()
        ...
        # irrelevant_info_generator_2 = torch.distributions.multivariate_normal.MultivariateNormal(torch.zeros((10,)), torch.eye(10)*0.5)
        #torch.distributions.dirichlet.Dirichlet(torch.ones(2)*0.5)
        ...
        ##add the isolated information to y
...
def generate_data_concepts_MI(num_data, num_concepts, feature_dim_info, label_dim_info, transform_dim = 100, MI=1.0,
                           transforms_2concept=None, transforms_2hd=None, noise=0.0, pos_prob=0.5):
    # Standard deviation of generated Gaussian distributions
    SEP = 0.5
    ...
    total_raw_features = OrderedDict()
    for k in feature_dim_info:
        total_raw_features[k] = []
    ...
    if transforms_2concept is None:
        ...
        for i in range(num_concepts):
            # transforms_2concept.append(np.random.uniform(0.0,1.0,(concepts_dims[i], 1)))
            transforms_2concept.append(mlp(concepts_dims[i]+512, 4, 1, layers=2, activation='tanh').to('cuda')) #concepts_dims[i]+512
    ...
    for data_idx in range(num_data):
        ...
        raw_features = dict()
        for k, d in feature_dim_info.items():
            # rho = np.sqrt(1 - np.exp(-2 * MI / d))
            # cov = np.eye(d)
            # cov[cov == 0] = rho
            # raw_features[k] = np.random.multivariate_normal(np.zeros((d,)), cov, (1,))[0]
            raw_features[k] = np.random.multivariate_normal(np.zeros((d,)), np.eye(d)*0.5, (1,))[0]
        ...
        concept_vector = [torch.cat((torch.tensor(raw_data[i], dtype=torch.float).to('cuda'), w3)) for i in range(num_concepts) ]
        concepts_labels = [transforms_2concept[i](concept_vector[i]).detach().cpu().numpy() for i in range(num_concepts)]
        ...
        raw_total = torch.tensor(raw_total).float().to('cuda')
        transforms_2hd.to('cuda')
        total_x = transforms_2hd(torch.cat((raw_total, w3, w4))).detach().cpu().numpy()
    ...
    total_raw_features['w3'] = w3_list
    total_raw_features['w4'] = w4_list
