...
class ConceptEncoder(pl.LightningModule):
    def __init__(self, backbone, num_classes, cfg_optim):
        ...
        self.projection = nn.Sequential(nn.Linear(self.num_features, self.embedding_dim), nn.ReLU())
        self.linear_2concept = nn.Linear(self.embedding_dim, self.num_classes) #nn.Linear(self.num_features, self.num_classes)
    ...
    def forward(self, x):
        z_c = self.backbone(x)
        z_c_ = self.projection(z_c.view(z_c.shape[0], -1))
        c = self.linear_2concept(z_c_.view(z_c_.shape[0], -1))
        return c, z_c_
    ...
    def configure_optimizers(self):
        self.params =  list(self.backbone.parameters()) + list(self.projection.parameters()) + list(self.linear_2concept.parameters())  #self.parameters()
    ...
    def training_step(self, batch, batch_idx):
        ...
        logits, _ = self(x)
        loss = F.binary_cross_entropy_with_logits(logits, c) #F.cross_entropy(logits, c.squeeze())#F.mse_loss(logits.float(), c) #
    ...
    def test_step(self, batch, batch_idx):
        x, y, c = batch[0], batch[1].squeeze(), batch[2].float()
        logits, _ = self(x)
        loss = F.binary_cross_entropy_with_logits(logits, c) #F.cross_entropy(logits, c.squeeze())#F.mse_loss(logits, c) #
        pred = torch.sigmoid(logits) #torch.softmax(logits, dim=-1)
