...
def main(cfg):
    ...
    run_name = str(cfg.model._target_) + time.strftime("_%Y_%m_%d") +('_9:1')
    ...
    for _ in range(cfg.num_eval):
        ...
        hydra.utils.log.info(f"Instantiating Trainer")
        val_loss = str(cfg.model._target_).split('.')[-1] + '/val_loss'
