...
################################################################################################################################
class MyDataModule2(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        indices_17 = np.where(np.isin(train_set.targets, [1,7]))[0]
        indices_9 = np.where(np.isin(train_set.targets, [9]))[0]
        indices = np.concatenate((indices_17[:int(len(indices_17)/20)], indices_9))
        # indices = np.concatenate((indices_17, indices_9[:int(len(indices_9)/4)]))

        train_set.targets = train_set.targets[indices]
        train_set.data = train_set.data[indices]
        # train_set.targets[train_set.targets==7] = 2
        # train_set.targets[train_set.targets==9] = 3
        
        test_set = ColoredMNIST_2('./', train=False, transform=None, download=True)
        
        indices = np.where(np.isin(test_set.targets, [1,7,9]))[0]
        test_set.targets = test_set.targets[indices]
        test_set.data = test_set.data[indices]

        # test_set.targets[test_set.targets==7] = 2
        # test_set.targets[test_set.targets==9] = 3
